...
export function AppNav({ onAdd }: AppNavProps) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-4 z-30 mx-auto flex w-fit items-center rounded-full bg-white/95 px-2 py-1.5 shadow-xl shadow-slate-900/15 ring-1 ring-slate-900/5 backdrop-blur">
      <Tab label="Wallet" Icon={WalletCardsIcon} active={onWallet} onSelect={() => navigate('/')} />

      <button
        onClick={onAdd}
        aria-label="Add card"
        className="relative z-10 mx-2 flex size-12 shrink-0 items-center justify-center rounded-full bg-blue-600 text-white shadow-lg shadow-blue-600/30"
      >
